    ...
    if (strpos($file_info['type'], 'image') === 0) {
        ...
        if ($file_info['type'] !== 'image/svg+xml') {
            ...
            if ($img_width > $max_width || $img_height > $max_height) {
                $file['error'] = 'Image dimensions exceed the allowed maximum (' . $max_width . 'px width and ' . $max_height . 'px height). Please consider optimizing the image for better site performance.';
            }
        }
        ...
        if ($file['size'] > $max_file_size_bytes) {
            ...
            $file['error'] = "Image size exceeds the allowed maximum ($max_file_size $file_size_unit). Please consider optimizing the image for better site performance.";
        }
    }
